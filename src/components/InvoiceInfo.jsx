...
function InvoiceInfo({cookies}) {
  ...
  const sendPDF = () => {
    ...
    emailjs.send('service_xxu45tj','template_o94bi2h', templateParams, 'u7cmwo4kt6eb6IzSC')
    .then((response) => {
  ...
  const [productQty, setProductQty] = useState(5); // State for productQty, initialized to 1
  const [subTotal, setSubTotal] = useState(100); // State for subTotal, initialized to 100
  const [grandTotal, setGrandTotal] = useState(114.98); // State for subTotal, initialized to 100
  ...
  return (
    <div className="bg-slate-200 h-[90%] flex p-10">
        ...
        <div id="pdfContainer" className="p-5" >
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div class="text-gray-700">
              ...
              <div class="text-sm">Due Date: {date.toLocaleDateString("en-US", dateOptions)}</div>
          ...
          <div class="border-b-2 border-gray-300 pb-8 mb-8">
            ...
            <div class="text-gray-700 mb-2">Aryan Arora</div>
            <div class="text-gray-700 mb-2">123 Main St.</div>
            <div class="text-gray-700 mb-2">Anytown, USA 12345</div>
            <div class="text-gray-700">aryanxarora@gmail.com</div>
          </div>
          ...
          <table class="w-full text-left mb-8">
            ...
            <tbody>
              {invoiceObj.invoiceDtls.map((product) => (
                <tr key={product.id}>
                  <td class="py-4 text-gray-700">Product #{product.id}</td>
                  <td class="py-4 text-gray-700">{productQty}</td>
                  <td class="py-4 text-gray-700">${(product.productPrice).toFixed(2)}</td>
                  <td class="py-4 text-gray-700">${(product.productPrice * productQty).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div class="flex justify-end mb-8">
            ...
            <div class="text-gray-700 font-bold text-xl">{grandTotal.toFixed(2)}</div>
          </div>
          ...
          <div class="border-t-2 border-gray-300 pt-8 mb-8">
            ...
            <div class="text-gray-700">821 Sainte Croix Ave, Saint-Laurent, Quebec H4L 3X9</div>
          </div>
      ...
      <div id="infoSection" className="basis-1/3 ml-10">
        {cookies.user.status ? 
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 mb-3 rounded-full" onClick={sendPDF}>Send Invoice to Client</button> :
          <button className="bg-yellow-500 hover:bg-yellow-400 text-white font-bold py-2 px-4 mb-3 rounded-full" onClick={downloadPDF}>Pay Invoice</button>
        }
        < br/>
        <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-full" onClick={downloadPDF}>Download PDF</button>
      </div>
    </div>
  );
}
